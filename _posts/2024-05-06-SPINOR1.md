...
이제 Clifford algebra의 representation을 구성해보자. Clifford algebra의 representation을 구성하면 spin group은 그 속에 자연스럽게 embedding 되어 있으므로 spin group의 representation을 얻을 수 있다. Explicit하게, Clifford algebra가 작용하는 공간의 원소인 spinor $\Psi$는 $\mathfrak{spin}(p,q)$의 임의의 element $\omega_{ab}J^{ab}$에 대해 $\Psi\rightarrow \frac{1}{2}\omega_{ab}\Gamma^{ab}\Psi$로 변한다 (Spin group은 Lie algebra에 exponential map을 취해 바로 얻어질 수 있으므로 이것이면 충분하다). 나아가 proposition 2.3.은 *generator들이 모두 Hermitian matrix로 표현될 때* Clifford algebra의 여러 element들을 $\Psi^\dagger(\cdots)\Psi$사이에 끼워넣으면 usual tensor transformation과 같이 변환함을 말해 준다. 예를 들어,
