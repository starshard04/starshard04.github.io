...
를 만족하고, 따라서 Lie algebra에 exponential map을 취해 얻어지는 spin group의 모든 원소는 $$\Gamma^{ab}/2$$를 통해 표현될 수 있다. 이때 $$\mathrm{SO}_0(p,q)$$의 어떤 원소를 생각한다면, spinor space에서의 transformation은 $$\Psi\rightarrow \frac{1}{4}\omega_{ab}\Gamma^{ab}\Psi$$이 됨에 유의하라; additional $1/2$ factor가 존재하는 이유는 double covering에 의해 spin group의 representation은 그 value가 대응되는 special orthogonal group의 representation의 2배가 되기 때문이다. Proposition 2.3.은 *generator들이 모두 Hermitian matrix로 표현될 때* Clifford algebra의 여러 element들을 $\Psi^\dagger(\cdots)\Psi$사이에 끼워넣으면 usual tensor transformation과 같이 변환함을 말해 준다. 예를 들어,
