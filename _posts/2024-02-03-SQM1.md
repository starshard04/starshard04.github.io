...
이 글에서는 초대칭의 존재를 가정함으로써 얻어질 수 있는 model-independent한 결과들에 대해 설명하고, 가장  단순한 example에서 이 결과들의 realization을 확인한다.
...
**Consequence 1.** **$H$는 nonnegative이다.** 임의의 state $|\Psi\rangle$에 대해,
...
둘 사이의 isomorphism은 $Q+Q^\dagger$를 통해 주어진다. 이를 보기 위해서는 $\mathcal{H}_{(n)}$에서 $(Q+Q^\dagger)^2=2E_n$임을 observe하면 된다. 단, ground state에서는 이는 성립하지 않으며 일반적으로 $\dim\mathcal{H}_{(0)}^B\ne\dim\mathcal{H}_{(0)}^F$이다.
...
$$
을 가해 canonically quantize하자. 이때 fermion operator들의 반교환 관계가 그냥 fermionic oscillator와 동일하므로 fermion들의 Hilbert space는 자연스럽게 $\{|0\rangle\}\oplus\{|1\rangle\}$가 되어, 전체 Hilbert space는
...
그 근본적인 이유는 supersymmetry algebra에 의해 (consequence 1.) 풀어야 하는 eigenvalue problem이 $H$에 관한 것이 아닌 $Q=Q^\dagger=0$의 *더 쉬운 문제*로 환원되기 때문이다. 이 경우, general state $|\Psi\rangle=f(x)|0\rangle+g(x)\overline{\psi}|0\rangle$가 $Q|\Psi\rangle=Q^\dagger|\Psi\rangle=0$을 만족할 조건은 다음과 같은 1차 선형 ODE

$$
(\partial_x+\partial h(x))f(x)=0,\quad (-\partial_x+\partial h(x))g(x)=0
$$

으로 환원되어 $e^{-h}|0\rangle$과 $e^h\overline{\psi}|0\rangle$의 선형 결합으로 아주 손쉽게 풀릴 수 있게 된다. 나아가, $h(x)$가 다항식인 경우를 생각해 보자. 그럼 다음과 같은 세 가지 case가 존재할 수 있다.
...
- $h(x)$의 degree가 짝수이고 leading coefficient가 양수인 경우, $e^{-h}|0\rangle$로 span되는 solution들은 규격화가 가능하다. 즉, 하나의 bosonic supersymmetric ground state가 존재하며, Witten index는 $1$이다. 이는 function $h(x)$의 degree와 leading coefficient의 부호 외에 어떤 detail에도 independent한 결과임에 주목하라!
- $h(x)$의 degree가 짝수이고 leading coefficient가 음수인 경우 supersymmetric ground state는 $e^h\overline{\psi}|0\rangle$으로 span된다. 하나의 fermionic supersymmetric ground state가 존재하고, WItten index는 $-1$이다.
