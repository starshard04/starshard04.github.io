...
**Definition 2.3.** *Smooth manifold $M,N$사이의 smooth map $f:M\rightarrow N$을 생각하자. 만약 $\mathrm{rank}\ \mathsf{d}f_x<\mathrm{dim}\ N$ i.e. differential map이 onto가 아니라면, $x$를 critical point라고 하고 그 image를 critical value라 한다. Critical point들의 집합 $C$에 대해 $N-f(C)$를 $f$의 regular value들이라 한다.*
