---
title: [토막글] 양자역학에서의 순간자(Instanton)
author: cj
date: 2024-02-09 02:05:00 +0900
categories: [shorts]
tags: [Instanton, Nonperturbative]
pin: true
math: true
mermaid: false
---
...
물론 다른 state사이의 transition amplitude도 계산할 수 있지만, energy splitting을 확인하는 목적에는 위의 식이면 충분하다. 둘 사이에 identity $\sum_n|n\rangle\langle n|$을 삽입하면,
...
이러한 configuration들은 앞서 말했듯이 두 minima 사이의 tunneling을 나타낸다. 이제 두 perturbative ground state $|+a\rangle$과 $|-a\rangle$사이의 transition amplitude를 이 instanton configuration을 중심으로 한 semiclassical path integral을 구성해 보자. general configuration을 
...
따라서, path integral을 통해 amplitude를 계산할 때 고려해야 할 새로운 saddle들이 무한히 많이 생겨난 것이다! 예를 들어, $x(-\infty)=-a$에서 $x(\infty)=a$로 가는 amplitude $\langle a|e^{-HT}|-a\rangle$를 계산한다고 하면, 1-instanton configuration의 $A_1$에 더해 3-instanton, 5-instaton...의 $A_3, A_5,\cdots$을 모두 고려해 주어야 한다.
...
위의 결과는 energy splitting을 explicit하게 나타내어 준다! Large $T$ limit에서 $T$에 대한 지수함수 $e^{-\omega T/2\pm KT e^{-I_{inst}}}$ 둘의 선형 결합으로 amplitde가 표현되므로, $|\pm a\rangle$의 두 perturbative ground state의 적절한 선형 결합으로 true ground state (with energy $\omega/2-Ke^{-I_{inst}}$)와 first excited state (with energy $\omega/2+Ke^{-I_{inst}}$)가 구성된다는 것을 알 수 있다; 앞서 미리 결과를 스포일러했듯이, nonperturbative한 deviation $\propto e^{-I_{inst}}$에 의해 두 동등해 보였던 perturbative ground state는 하나의 true ground state와 하나의 excited state를 낳는다.
