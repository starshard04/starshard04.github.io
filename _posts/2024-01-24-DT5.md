...
**Definition 1.1.** *Compact $n$-manifold $M$와 두 submanifold $N,N'$을 생각하자. $\partial M=\partial N=\partial N'=\empty$일 때, 만약 $M\times[0,1]$의 subset*
...
$$
\mathcal{U}|_{N\times [0,\epsilon)}(x,t)=(b^i(x),0),\quad \mathcal{U}|_{N'\times (1-\epsilon,1]}(x,t)=(b'^i(x),0),
$$
...
Theorem 1.5.을 증명하기 위해서는 우선 다른 하나의 theorem을 증명해야 한다. $(N,\mathcal{B})$가 codimension-$p$ compact framed submanifold of $M$이고 $\partial M=\partial N=\empty$라 하자.
